...
const FaqTable = ({callback, setCallback}: {callback:boolean, setCallback:any}) => {
  ...
  return (
    <div className="overflow-x-auto">
      <Table className="w-full min-w-[500px]">
          ...
          {loading ?
            ...
            <>
                  {displayedFaqs?.map((faq, index) => {
                    return (
                      <TableRow key={index} className=" w-full border-b border-slate-300 p-4 text-sm font-medium text-black *:text-black last-of-type:border-b">
                        <TableCell>
                          {faq.question}
                        </TableCell>
                        ...
                        {isDialogOpen && <DeleteDialog onClose={handleCloseDialog} faqs={faq} callback={callback} setCallback={setCallback} />}
                        {updateModal && <UpdateFaqModal onClose={handleCloseUpdateDialog} faqs={faq} callback={callback} setCallback={setCallback} />}
                      </TableRow>
                    );
                  })}
            </>
        ...
        {!loading && faqs?.length === 0 && <p className="text-center mt-40">No data available</p>}
