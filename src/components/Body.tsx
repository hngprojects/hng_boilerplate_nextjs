...
export const Body = () => {
  ...
  return (
    <div className='flex flex-col gap-[24px]'>
        <h1 className='text-[#525252] text-[28px] font-bold md:text-center'>Latest Articles</h1>
        <div className='flex flex-col items-start gap-[24px] mb-3 md:w-[70%] md:mx-auto'>
            {
                mainBlog.slice(0, visible).map(blog=> {
                    return(
                        <div key={blog.id} className='flex flex-col md:flex-row md:gap-[24px] md:p-[24px] justify-center items-start border-b border-[#525252]'>
                            <div className=''>
                                ...
                                <div className='flex items-center gap-[6px]'>
                                    <Image width={24} src={blog.accountImgMobile} alt='image'/>
                            ...
                            <div className='relative'>
                                <Image src={blog.image} alt='picture'></Image>
                            </div>
                        </div>
                    )
                })
        ...
        <button onClick={showMoreItem} className="w-full md:w-[20%] lg:w-[11%] md:mx-auto bg-[#F97316] rounded-md text-center text-[14px] font-medium text-primary-foreground py-[9px] leading-[25px]">{loading ? 'LOADING' : "Load More"}</button>
    </div>
  )
}
