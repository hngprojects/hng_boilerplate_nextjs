...
describe("Accordion Component", () => {
    it("renders without crashing", () => {
        ...
        expect(screen.getByText("Is there a discount for annual subscriptions?")).toBeInTheDocument();
        expect(screen.getByText("Do you offer a free trial?")).toBeInTheDocument();
    });

    it("expands and collapses content on click", () => {
        render(<Accordion items={items} />);
        const firstItemHeader = screen.getByText("What payment methods do you accept?");
    ...
    it("only one item is expanded at a time", () => {
        render(<Accordion items={items} />);
        const firstItemHeader = screen.getByText("What payment methods do you accept?");
        const secondItemHeader = screen.getByText("Is there a discount for annual subscriptions?");
        const firstItemContent = screen.queryByText("Answer to question 1.");
