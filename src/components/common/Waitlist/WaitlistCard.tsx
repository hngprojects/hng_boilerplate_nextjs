...
export type WaitlistCardProps = CardProps & {
  cardTitle: string;
...
const WaitlistCard = ({
  ...
  return (
    <Card
      className={cn(
        `md:w-[320px] w-[100%] bg-[#fafafa] ${waitlistBorderStyle}`,
        className,
      )}
      {...props}
    >
      <CardHeader className={cn("p-24px")}>
        ...
        <CardTitle role="cardTitle" className={cn("text-[18px] text-[#525252]")}>
          {cardTitle}
        </CardTitle>
        <CardDescription role="cardDescription" className={cn("text-[14px] leading-[20px]")}>
          {cardDescription}
        </CardDescription>
      </CardHeader>
    </Card>
